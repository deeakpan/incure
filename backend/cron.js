...
const MUTATION_ABI = [
  ...
  'function lastMutationTime() view returns (uint256)',
];
const FORMULA_ABI = [
...
export function startCronJobs(gameAddress, rpcUrl, deployerKey) {
  ...
  const game = new ethers.Contract(gameAddress, [...SPREAD_ABI, ...MUTATION_ABI, ...FORMULA_ABI], wallet);
  ...
  cron.schedule('*/5 * * * *', async () => {
    try {
      ...
      console.log('📊 Data published to Somnia Data Streams by contract');
    } catch (error) {
